...
module Jos19
  module UnificarHelper
    ...
    def arreglar_casos_medio_borrados
      ...
      mens = if numpora == 0
        "No hay casos parcialmente eliminados.\n"
      else
        "De los #{numpora} casos parcialmente eliminados, se completaron #{lcom.count} (i.e #{lcom.join(", ")}) y se eliminaron #{lelim.count} que no tenían beneficiarios o cuyos beneficiarios estaban en otros casos (i.e #{lelim.join(", ")}).\n"
      end
      mens
    end
    ...
    def eliminar_personas_en_blanco
      ...
      mens = if lpore.count == 0
        "No hay beneficiarios en blanco.\n"
      else
        "Se eliminaron #{lpore.count} beneficiarios en blanco no asociadas a casos ni a actividades (#{lpore.join(", ")}).\n"
      end
      mens
    end
